...
export default function AgentInfoScreen({route}) {
  ...
  useEffect(async() => {
  const data = await getAgentsByUuid(route.params.uuid, lang.es)
  setagent(data)
    ...
    <View style={{width:'100%',flex:1}}>
      {isLoading?<LoadingScreen/>:
        <ScrollView>
          ...
          <Text style={{color:secondaryColor,fontSize:15,fontWeight:'bold',letterSpacing:2}}>// BIOGRAFÍA</Text>
          <Text style={{color:'white',marginTop:10,lineHeight:20}}>
            {agent.description}
          </Text>
          {/* Agent Abilities */}
          <Text style={{color:secondaryColor,fontSize:15,fontWeight:'bold',letterSpacing:2,marginTop:20}}>// HABILIDADES</Text>
